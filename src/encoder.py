...
class object2vec_encoder:
    '''in silico model with image features as input and fMRI beta values as output
    '''
    def __init__(self, subject, rois):    
        self.subject = subject
        self.rois = rois
        root = '/data/chan21/semanticdimensionality/data/'
        roistack = scipy.io.loadmat(root + 'fmri/object2vec/subj{:03}'.format(self.subject) +
        ...
        sets = scipy.io.loadmat(root + 'fmri/object2vec/subj{:03}'.format(self.subject) +
                                   '/sets.mat')['sets']
